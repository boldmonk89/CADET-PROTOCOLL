...
export default function AuthPage() {
    ...
    try {
      if (mode === "signup") {
        ...
        if (data.session) {
          ...
          toast.success("Registration success! Check your email to confirm identity.");
          setMode("signin");
        }
      } else {
  ...
  const handleGoogle = async () => {
    setLoading(true);
    try {
      // Removing redirectTo so it uses the Supabase Default Site URL, bypassing 'requested path invalid' errors.
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 bg-background overflow-hidden">
      ...
      <div className="absolute top-0 inset-x-0 h-[300px] bg-gradient-to-b from-primary/5 to-transparent blur-3xl pointer-events-none"></div>
      ...
      >
        ...
        <p className="mt-8 text-center text-[9px] font-mono-tac uppercase tracking-[0.3em] text-muted-foreground/60">
